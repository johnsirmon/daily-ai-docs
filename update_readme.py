...
Fetches the latest release from each tracked GitHub repository and updates
the ``<!-- LATEST-UPDATES-START --> … <!-- LATEST-UPDATES-END -->`` block
inside README.md.
...
Options:
    --dry-run   Print what would be written; do not modify README.md.
...
def build_section(releases: list[dict]) -> str:
    ...
    lines: list[str] = [
        ...
        f"",
        f"| Project | Latest Release | Date | Notes |",
        f"|---------|---------------|------|-------|",
    ]
...
def update_readme(section_text: str, readme_path: Path, dry_run: bool) -> None:
    ...
    if pattern.search(content):
        ...
        # Sentinels not present — prepend the section after the first heading
...
def main() -> None:
    ...
    releases: list[tuple[str, dict | None]] = []
    for owner, repo, label in TRACKED_REPOS:
        release = fetch_latest_release(owner, repo)
        status = release["tag_name"] if release else "not found"
