...
def render_manifest_readme(manifest, feed_url: str | None = None) -> str:
    """Render the current daily brief from the same manifest used for audio."""
    ...
    lines = [
        ...
        "",
        "A concise, source-backed daily briefing for AI agent developers: what changed, why it matters, what is worth learning, and whether to act, watch, or skip.",
        "",
        "## Podcast",
        "",
        f"Follow this RSS feed in Apple Podcasts: `{feed_url}`",
        "",
        "Apple Podcasts on iPhone: Library → Follow a Show by URL. The followed show is then available in CarPlay.",
        "",
        "## Today's signal",
        "",
    ]
    ...
    lines += [
        ...
        "- The weekly YouTube digest may contribute at most one transcript-backed learning recommendation; it never replaces vendor evidence.",
        "",
        "## Source health",
        "",
        *[f"- `{name}`: {status}" for name, status in sorted(manifest.source_health.items())],
        "",
        "_Generated by the source in this repository. A green workflow is not considered published until the public feed and audio pass health checks._",
        "",
        "[Operations and setup](docs/OPERATIONS.md)",
        "",
        "## Weekly YouTube signal",
        "",
        "A Sunday workflow uses age-adjusted view velocity, a channel/global baseline, and transcript usefulness to retain five compact learning candidates. Full transcripts are discarded, and daily episodes deduplicate video IDs.",
        "",
    ]
    return "\n".join(lines)
